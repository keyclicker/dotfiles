...
an explicit `--allow-root`. The NixOS service allows `~/monorepo/AI/` and
`~/projects/`. Create a symlink in `~/public/` to publish a selected directory
or file; allowlisting a root does not publish it automatically.
